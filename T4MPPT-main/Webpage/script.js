...
function handleCSVResult(csvString) {
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    var cells = row.split(',');
	if(i==0) {
		var currentVolt = cells[0];
		var currentAmp = cells[1];
		var currentPower = cells[2];
		var currentTime = cells[3];
		var currentTemp = cells[4]; 	
	}
  ...
  htmlStr += '<p>Last updated on '+ currentTime+ '</p>' + '<p>Voltage: ' + currentVolt + 'V</p><p>Current: ' + currentAmp + 'A</p><p>Power: ' + currentPower + 'W</p><p>Temperature: '+ currentTemp + '</p><br>';
